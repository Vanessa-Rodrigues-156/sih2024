<template>
    <div class="min-h-screen bg-slate-900 p-6">
        ...
        <div class="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart :data="graphData">
              ...
              <Line type="monotone" dataKey="line2" stroke="#ef4444" :strokeWidth="2" :dot="false" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <div class="bg-slate-800 rounded-lg p-6">
          ...
          <div class="space-y-4">
            <div v-for="(item, index) in incidentTypes" :key="index" class="flex justify-between items-center">
        ...
        <div class="bg-slate-800 rounded-lg p-6">
          ...
          <div class="space-y-4">
            <div v-for="(item, index) in sectors" :key="index" class="flex justify-between items-center">
              <span class="text-slate-400">{{ item.type }}</span>
              <span class="text-slate-100">{{ item.count }}</span>
            </div>
          </div>
        ...
        <div class="bg-slate-800 rounded-lg p-6">
          ...
          <div class="space-y-4">
            <div v-for="(item, index) in locations" :key="index" class="flex justify-between items-center">
              <span class="text-slate-400">{{ item.type }}</span>
              <span class="text-slate-100">{{ item.count }}</span>
            </div>
          </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-slate-800 rounded-lg p-6">
          ...
          <div class="space-y-4">
            <div v-for="(incident, index) in recentIncidents" :key="index" class="flex items-start space-x-3">
  ...
  const graphData = Array.from({ length: 12 }, (_, i) => ({
    name: i,
    line1: Math.sin(i / 2) * 50 + 50,
    line2: Math.cos(i / 2) * 50 + 50,
  }))
